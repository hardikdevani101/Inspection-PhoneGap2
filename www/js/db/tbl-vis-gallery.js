Tbl_VISGallery = function(dbstore){
	this.app = app;
			...
			function(tx) {
				...
				if (fileInfo.X_INSTRUCTIONLINE_ID == 0
					...
					sqlQuery = 'INSERT INTO vis_gallery(mr_line,insp_line,name,file) VALUES ("'
							...
							+ fileInfo.fileName
							+ '","' + fileInfo.fileFullPath + '")';
				tx.executeSql(sqlQuery, [], sucessCallback, errorCallback);
			}, errorCallback);
...
Tbl_VISGallery.prototype.getFilesByMRInfo=function(fileInfo, sucsses,error){
	...
	this.dbstore.transaction(function(tx) {
		var sqlQuery = 'SELECT * FROM vis_gallery WHERE mr_line="'
			+ fileinfo.M_InOutLine_ID + '" and in_out_id="' + fileinfo.M_INOUT_ID
			+ '"';
...
Tbl_VISGallery.prototype.getFilesToBeDeleted = function(fileInfo,success,error) {
	var successCallback = this.dbstore.success;
	if(typeof(success) === "function"){
		successCallback=success;
	}
	...
	this.dbstore.transaction(getFilesAttachedF, errorCallback);
...
Tbl_VISGallery.prototype.getUploadCounts = function(fileInfo, success,error) {
	...
	this.dbstore.transaction(function(tx) {
		...
		if (fileInfo.isInsp == 0)
			sqlQuery = 'SELECT * FROM vis_gallery WHERE mr_line="' + fileInfo.mInNumber
					+ '" and in_out_id="' + fileInfo.InspNumber + '"';
		else
			sqlQuery = 'SELECT * FROM vis_gallery WHERE mr_line="' + fileInfo.mInNumber
					+ '" and insp_line="' + fileInfo.InspNumber + '"';
		tx.executeSql(sqlQuery, [], function(tx, results) {
			var totImg = results.rows.length;
			if (fileInfo.isInsp == 0)
				sqlQuery = 'SELECT * FROM vis_gallery WHERE mr_line="'
						+ fileInfo.mInNumber + '" and in_out_id="' + fileInfo.InspNumber
						+ '" and imgUpload="T"';
			else
				sqlQuery = 'SELECT * FROM vis_gallery WHERE mr_line="'
						+ fileInfo.mInNumber + '" and insp_line="' + fileInfo.InspNumber
						+ '" and imgUpload="T"';
			tx.executeSql(sqlQuery, [], function(tx, results) {
...
Tbl_VISGallery.prototype.updateFileOnSuccess= function(fileInfo, success, error) {
	var successCallback = this.dbstore.success;
	if(typeof(success) === "function"){
		successCallback=success;
	}
	...
	this.dbstore
			.transaction(updateFileinfo,successCallback
					, errorCallback);
...
Tbl_VISGallery.prototype.removeOtherFiles = function(fileInfo,success,error) {
	...
	this.dbstore.transaction(function(tx) {
		var sqlQuery;
		if (fileInfo.X_INSTRUCTIONLINE_ID == 0 || fileInfo.X_INSTRUCTIONLINE_ID == null)
			sqlQuery = 'DELETE FROM vis_gallery WHERE file="' + fileInfo.tempfile
					+ '" and in_out_id="' + fileInfo.M_INOUT_ID + '"';
		else
			sqlQuery = 'DELETE FROM vis_gallery WHERE file="' + tmpfile
					+ '" and insp_line="' + fileInfo.X_INSTRUCTIONLINE_ID + '"';
		tx.executeSql(sqlQuery,errorCallback);
	}, successCallback, errorCallback);
}

Tbl_VISGallery.prototype.updateFileOnUploadFailure= function(fileInfo, success, error) {
	var successCallback = this.dbstore.success;
	if(typeof(success) === "function"){
		successCallback=success;
	}
