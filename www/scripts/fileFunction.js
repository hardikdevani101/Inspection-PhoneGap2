...
function saveImage(imageURI) {
    ...
    dirVISInspection.getFile(fileName, {create: true, exclusive: false}, function (fileEntry) {
        ...
        console.log("File Create FSError = " + error.code);
    });
}
...
function CreateImgWriter(fileEntry, imageURI) {
    ...
    fileEntry.createWriter(function (writer) {
        ...
        console.log("file entry FSError = " + error.code);
    });
}
    ...
    DirReader.readEntries(function (entries) {
        ...
        for (var i = 0; i < entries.length; ++i) { // sort entries
            ...
            if (entry.isDirectory && entry.name[0] != '.') {
                ...
                var tmpStr1 = "listSub('" + entry.name + "')";
                div.setAttribute('onclick', tmpStr1);
                div.innerHTML = entry.name;
            }
            else if (entry.isFile && entry.name[0] != '.') {
                div.className = "file";
                var tmpStr2 = "onFileExplorerClick('" + entry.fullPath + "')";
                div.setAttribute('onclick', tmpStr2);
                div.innerHTML = entry.name;
            }
            dirContent.append(div);
        }
