...
function onOrgChange() {
    ...
    if (org_id == 0) {
        for (var i = 0; i < warehouseListArray.length; i++) {
            ...
            option1.text = warehouseListArray[i][2];
            option1.value = warehouseListArray[i][1];
            if (option1.value == vis_whouse_id)
                option1.selected = true;
            wareHouseDrop.add(option1);
        }
    }
    else {
        for (var i = 0; i < warehouseListArray.length; i++) {
            if (warehouseListArray[i][0] == org_id) {
                ...
                option2.text = warehouseListArray[i][2];
                option2.value = warehouseListArray[i][1];
                if (option2.value == vis_whouse_id)
                    option2.selected = true;
                wareHouseDrop.add(option2);
            }
        }
    }
}
    ...
    if (imgUploadCount == 0) {
        ...
        function attachimageSelect(tx, result) {
            if (result.rows.length == 0) {
                ...
                navigator.notification.alert('Failed to upload all files, Try again', function () {
                }, 'Failure', 'OK');
                onStopNotification();
            }
        }
...
function deleteImageSelectSuccess(tx, results) {
    for (var i = 0; i < results.rows.length; i++) {
        if (DataTypes.indexOf(getExtension(results.rows.item(i).name).toUpperCase()) >= 0) {
    ...
    onStopNotification();
    navigator.notification.alert('All Files attached', function () {
    }, 'Success', 'OK');
}
...
function deleteSelectedGallery() {
    ...
    for (var j = 0; j < imagelistarray.rows.length; j++) {
        if (DataTypes.indexOf(getExtension(getFileName(imagelistarray.rows.item(j).file)).toUpperCase()) < 0) {
...
function fillGalleryPhotos() {
    ...
    for (var j = 0; j < imagelistarray.rows.length; j++) {
        if (DataTypes.indexOf(getExtension(imagelistarray.rows.item(j).name).toUpperCase()) < 0) {
        ...
        db.transaction(function (tx) {
            tx.executeSql('SELECT * FROM vis_gallery WHERE mr_line="' + M_InOutLine_ID + '"', [],
                function (tx, results) {
                    ...
                    fillMrLines();
                    navigator.notification.alert('Discard Inspection data success', function () {
                    }, 'Success', 'OK');
                }, function (err) {
                    console.log("Error SQL: " + err.code);
                });
